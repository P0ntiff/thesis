...
Doshi-Velez \& Kim (2017) provided an influential set of guidelines or ``evaluation paradigms'' for evaluating interpretability techniques, arranged in three levels of increasing abstraction \cite{paradigms}. Their motivation was to provide a scientific basis for the different approaches to evaluation that method authors implicity use (both qualitative and quantitative). The three paradigms are based around what the authors highlight are different objectives in evaluation (in order of increasing abstraction):
...
\item \textbf{Human-grounded}: How much everyday people agree that an explanation is visually superior to another: user studies are one example.
...
Research on evaluation criteria below (Section \ref{sec:existing_criteria}) has focused on functionally-grounded criteria, since these are dominant in the literature, but also since the higher level paradigms above are difficult to compare objectively.
...
\subsubsection{Towards Better Understanding of Gradient-based attribution methods \textbf{cite ancona}}
Similar to SHAP in motivation, an attempt at providing a unified framework for the gradient-based class of attribution methods was provided by Ancona et al. (2018).

\subsubsection{Model-Agnostic Interpretability of Machine Learning}
