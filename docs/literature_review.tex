...
Some authors sidestep the functionally-grounded approach to method evaluation to suggest ``axioms'' that fall more under the human-grounded paradigm. These criteria are summarised in Table 1.X from lower to higher level criteria:

consistency between models
input invariance


Across in the model-agnostic camp Ribeiro et al. (2016) have made the case for model flexibility as a strong criteria for explanation technique usability \cite{case_for_model_ag}. When comparing a tree-based model with a deep learning model, for example, neither a practitioner nor a researcher can get a fair idea of interpretability if two explanations are different in representation on account of different attribution methods. The observation by Ancona et al. (2017) that more complex architectures are not explained as well by current model-specific techniques also supports this argument (Section \ref{sec:existing_studies}) \cite{ancona}.
...
There are existing frameworks on researcher GitHubs that offer practitioners and researchers a panel of attribution methods to choose from. 
...
There are no explanation frameworks for collecting both model-agnostic and model-specific techniques for saliency maps. These software packages are also typically combining only a few methods from one approach, particularly the gradient-based methods and their derivatives. Additionally, no software evaluation framework for WSOL-based saliency metric evaluation is known to be available.
