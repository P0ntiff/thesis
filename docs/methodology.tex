...
Visual examples of the methods made compatible for evaluation is shown in Figure X over the page. Some brief comments are made here about work necessary for each method and the insights gained:
...
The motivation to create a new variant of an intersection-over-union metric, rather than rely on existing bounding box based comparison metrics, was based on observations in Related Work that existing metrics do not account for pixel-level weights, and punish perforated saliency maps because of their reliance on a single component to find the box.

% formula ??

Note the design of a proxy metric was only meant to provide some objective basis for evaluating methods in the image classification context. Other analysis (qualitative) was also performed for testing methods on higher level criteria than only saliency `quality'.

A variant of the metric was also implemented to account for intensity of attribution (weights on each pixel). This was a crude measure that got mixed results, but 
%Localisation performance was compared to a baseline?
...
This evaluator class can implement other saliency metrics. This is mentioned in Future Work
